	...
	private void loadCoaches(ObservableList<TempUser> coaches) {
		try {
			...
			for (String s : allCoaches.keySet()) {
				coaches.add(new TempUser(allCoaches.get(s).getUsername(), allCoaches.get(s).getName(), allCoaches.get(s).getCity(),
						Integer.toString(allCoaches.get(s).getAge()), "Random shit"));
			}
		}
		catch (Exception e) {
